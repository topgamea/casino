...
func TestConfig(t *testing.T) {
	...
	assert.Equal(t, []string{"6", "7", "8", "6", "7", "8", "6", "7", "8"}, c.BoardsConfig["3"].Slots, "boards config one board slots not matched")
	assert.Equal(t, []string{"6", "7", "8", "6", "7", "8", "6", "7", "8"}, c.BoardsConfig["4"].Slots, "boards config one board slots not matched")
	...
	//check wild config
	assert.Equal(t, []int{7}, c.WildConfig.IDs, "wild ids not matched")
	assert.Equal(t, []int{6}, c.WildConfig.Except, "wild except not matched")
}
